package io.github.emir4169.reshex.config
...
object ReshexClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            ReshexServerConfig.holder.config = ReshexServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            ReshexServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Reshex.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use ReshexServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = ReshexServerConfig.ServerConfig()
    }
